...
"聚合（mere aggregation）"主张基于以下事实：本组件为独立编译的
STATIC 库 target（`quemusic_meshgradient`），拥有独立的源码目录与
许可证，主体经 Qt Quick / QML 公开接口调用。

需要说明的是：主流解释认为，主程序与其动态加载/链接的库之间若存在
API 级依赖，可能被认定为派生作品（derivative work）而非聚合；
若该解释成立，QueMusic 整体分发物需遵循 AGPL-3.0。

后续可选方案（由维护者决策）：
1. 以宽松许可（MIT / Apache-2.0 / BSD）自行实现替代的网格渐变背景；
2. 将项目整体许可变更为 AGPL-3.0；
3. 维持现状并寻求正式法律意见。
